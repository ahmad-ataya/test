...
var async = require('async'),
    ...
    buildFindParameters = function (params) {
        ...
        searchableFields.forEach(function (field) {
            var pathKey = Models.sms.schema.path(field);  //ataya
            if(!pathKey)
                return;
            var orCondition = {};
            if (pathKey.instance == 'ObjectID'){
                if(mongoose.Types.ObjectId.isValid(_.padEnd(searchText, 24,'0')))
                    orCondition[field] = _.padEnd(searchText, 24,'0');
                else
                    orCondition[field] = '000000000000000000000000'; // not ID
            }
            else if(pathKey.instance == 'Date'){
                if(!isNaN(new Date(searchText)))
                    orCondition[field] = new Date(searchText);
            }
            else
            ...
            return new Promise(function (fullfill, reject) {
                ...
                async.series([
                    ...
                    function fetchRecordsTotal (cb) {
                        Model.count(params.extraWhere, function (err, count) {
                            if (err) {
                                return cb(err);
                            }
                            recordsTotal = count;
